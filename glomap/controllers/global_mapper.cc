...
bool GlobalMapper::Solve(const colmap::Database& database,
  ...
  if (!options_.skip_rotation_averaging) {
    ...
    RelPoseFilter::FilterRotations(view_graph, images, options_.inlier_thresholds.max_rotation_error);
    view_graph.KeepLargestConnectedComponents(images);
    ...
    LOG(INFO) << num_img << " / " << images.size() << " images are within the connected component." << std::endl;
  ...
  if (!options_.skip_track_establishment) {
    ...
    LOG(INFO) << "Before filtering: " << tracks_full.size() << ", after filtering: " << num_tracks << std::endl;
